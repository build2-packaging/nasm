...
top_srcdir="$(readlink -f $(dirname $makefile))"
top_srcdir="$(git -C "$top_srcdir" rev-parse --show-toplevel)"
srcdir="$top_srcdir"
objdir="$top_srcdir"
prefix=/build
exec_prefix=${prefix}
...
for file in ${makefiles[@]}; do
  ...
  while read -r line; do
    ...
    for prereq in ${__prereqs[@]}; do
      [[ -f "$top_srcdir/$prereq" ]] && touch "$top_srcdir/$prereq"
    done
